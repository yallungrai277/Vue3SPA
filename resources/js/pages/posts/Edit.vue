...
const { validationErrors, isFormSubmitting, updatePost, getPost, post } =
  usePosts();
...
onMounted(() => {
  getCategories();
  getPost(router.params.id).then(() => {
    _.assign(form, _.pick(post.value, _.keys(form))); //assign only if property exists in target object
  });
});
